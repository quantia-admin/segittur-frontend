...
function Costos() {
  ...
  // Cargar los datos de la tabla `costos`
  const loadCostos = () => {
    axios.get(`${API_URL}/costos`)
      .then(response => {
        setRows(response.data);
        enqueueSnackbar('Costos obtenidos correctamente', { variant: 'success' });
      })
      .catch(error => {
        console.error('Error al obtener los datos de costos:', error);
        enqueueSnackbar('Error al obtener los costos', { variant: 'error' });
      });
  };
  ...
  const handleDownloadExcel = () => {
    ...
    link.href = '/templates/costo_template.xlsx';
    link.setAttribute('download', 'costo_template.xlsx');
  ...
  const handleUploadExcel = async () => {
    ...
    try {
      const response = await axios.post(`${API_URL}/upload-excel?table=costos`, formData, {
        headers: {
      ...
      if (response.status === 200) {
        ...
        loadCostos(); // Recargar los datos después de la subida
      }
    } catch (error) {
  ...
  // Función para eliminar un registro de costos
  const handleDeleteRow = (id) => {
    axios.delete(`${API_URL}/costos/${id}`)
      .then(() => {
        setRows(rows.filter(row => row.id !== id));
        enqueueSnackbar('Costo eliminado exitosamente', { variant: 'success' });
      })
      .catch(error => {
        console.error('Error al eliminar el costo:', error);
        enqueueSnackbar('Error al eliminar el costo', { variant: 'error' });
      });
  };
  ...
  const columns = [
    ...
    { field: 'monto', headerName: 'Monto (€)', minWidth: 150, flex: 1, type: 'number' },
    { field: 'tipo_costo', headerName: 'Tipo de Costo', minWidth: 150, flex: 1 },
    {
      field: 'actions',
      headerName: '',
      minWidth: 50,
  ...
  return (
    <>
      <div className="content">
        <Card>
          <CardHeader>
            <CardTitle tag="h4">Gestión de Costos</CardTitle>
          </CardHeader>
          <CardBody>
