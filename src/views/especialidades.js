...
function Especialidades() {
  ...
  // Cargar los datos de la tabla `especialidad`
  const loadEspecialidades = () => {
    axios.get(`${API_URL}/especialidades`)
      .then(response => {
        setRows(response.data);
        enqueueSnackbar('Especialidades obtenidas correctamente', { variant: 'success' });
      })
      .catch(error => {
        console.error('Error al obtener los datos de especialidades:', error);
        enqueueSnackbar('Error al obtener las especialidades', { variant: 'error' });
      });
  };
  ...
  const handleUploadExcel = async () => {
    ...
    try {
      const response = await axios.post(`${API_URL}/upload-excel?table=especialidad`, formData, {
        headers: {
      ...
      if (response.status === 200) {
        ...
        loadEspecialidades(); // Recargar los datos después de la subida
      }
    } catch (error) {
  ...
  const handleDeleteRow = (id) => {
    axios.delete(`${API_URL}/especialidades/${id}`)
      .then(() => {
        setRows(rows.filter(row => row.id !== id));
        enqueueSnackbar('Especialidad eliminada exitosamente', { variant: 'success' });
      })
      .catch(error => {
        console.error('Error al eliminar la especialidad:', error);
        enqueueSnackbar('Error al eliminar la especialidad', { variant: 'error' });
      });
  };
  ...
  const handleDownloadExcel = () => {
    ...
    link.href = '/templates/especialidad_template.xlsx';
    link.setAttribute('download', 'especialidad_template.xlsx');
    document.body.appendChild(link);
    link.click();
    document.body.removeChild(link);
  };

  // Columnas de la tabla `especialidad`
  const columns = [
  ...
  return (
    <>
      <div className="content">
        <Card>
          <CardHeader>
            <CardTitle tag="h4">Gestión de Especialidades</CardTitle>
          </CardHeader>
          <CardBody>
